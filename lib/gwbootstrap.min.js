...
jQuery(window).on('load',()=>{matchPageTopToNavbar();matchFooterHeight();matchFloatingButtons()});jQuery(window).resize(()=>{matchPageTopToNavbar();matchFooterHeight();matchFloatingButtons()});jQuery(document).ready(()=>{jQuery('.btn-table').click(exportTableToCSV);jQuery('.image-switch').click(showImage);jQuery('#top-btn').click(function(){jQuery(this).scrollView()});jQuery('.lazy').Lazy({effect:'fadeIn',onError:element=>{console.log(`error loading ${element.data('src')}`)}});jQuery('.fancybox').fancybox({backFocus:false,buttons:['zoom','share','slideShow','download','thumbs','close'],helpers:{title:{type:'inside'}},width:1200})});
