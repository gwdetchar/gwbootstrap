...
jQuery(window).on("load",()=>{matchPageTopToNavbar();matchFooterHeight();matchFloatingButtons()});jQuery(window).resize(()=>{matchPageTopToNavbar();matchFooterHeight();matchFloatingButtons()});jQuery(document).ready(()=>{jQuery(".btn-table").on("click",exportTableToCSV);jQuery(".image-switch").on("click",showImage);jQuery("#top-btn").on("click",function(){jQuery(this).scrollView()});jQuery(".lazy").Lazy({effect:"fadeIn",effectTime:1000,visibleOnly:true,scrollDirection:"vertical",onError:element=>{console.log(`Error loading ${element.data("src")}`)}});Fancybox.bind("[data-fancybox=\"gallery\"]",{contentClick:"toggleZoom",placeFocusBack:false,Images:{initialSize:"fit"},Thumbs:{showOnStart:false,type:"classic"},Toolbar:{display:{right:["toggleZoom","download","slideshow","thumbs","close"]}}})});
